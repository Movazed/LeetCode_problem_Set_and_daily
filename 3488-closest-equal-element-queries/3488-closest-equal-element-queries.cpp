...
/****************** Prime Generator End **********************/ 
#define CALC_DIFF(q, vec, n, ans) { \
    int idx = lower_bound(all(vec), q) - vec.begin(); \
    int next = vec[(idx + 1) % sz(vec)]; \
    int prev = vec[(idx - 1 + sz(vec)) % sz(vec)]; \
    int diff1 = abs(q - next); \
    int diff2 = abs(q - prev); \
    diff1 = min(diff1, n - diff1); \
    diff2 = min(diff2, n - diff2); \
    ans.pb(min(diff1, diff2)); \
}
/****************** Geometry *****************/ 
...
class Solution {
public:
    vector<int> solveQueries(vi& nums, vi& queries) {
        int n = sz(nums);
        umap<int, vi> pos;
